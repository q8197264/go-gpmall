...
func OpenTracingClientInterceptor(tracer opentracing.Tracer, optFuncs ...Option) grpc.UnaryClientInterceptor {
	...
	return func(
		...
		// -- begin --
		// 个人修改: 解决gin集成的 tracer 与 parentSpan 传递 的问题
		ginCtx := ctx.Value("ginContext")
		if ginContext, ok := ginCtx.(*gin.Context); ok {
			if iTracer, ok := ginContext.Get("tracer"); ok {
				tracer = iTracer.(opentracing.Tracer)
			}
			if iParentCtx, ok := ginContext.Get("parentSpan"); ok {
				parentCtx = iParentCtx.(opentracing.Span).Context()
				// parentCtx = iParentCtx.(*jaeger.Span).Context()
			}
